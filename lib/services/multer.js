...
const fileFilter = (formats, size) => (req, file, cb) => {
  ...
  if (fileSize > size) {
    const err = new Error(`Only files lower than ${size / (1024 * 1024)}mo are allowed`);
    err.code = 'SERVICE_ERROR';
    return cb(err, false);
  }
  if (!formats.includes(file.mimetype)) {
    const err = new Error(`Only ${formats} files allowed`);
    err.code = 'SERVICE_ERROR';
    return cb(err, false);
  }
  return cb(null, true);
...
const create = (config) => async (req, res, next) => {
  ...
  // upload
  const upload = multer(options).single(config.kind || 'img');
  upload(req, res, (err) => {
    if (err) {
      req.multerErr = err;
    }
