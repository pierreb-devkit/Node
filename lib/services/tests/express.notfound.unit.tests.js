...
describe('express initModulesServerRoutes — content-negotiated 404 (#3975, #3978):', () => {
   ...
   * isolation. Shared by getApp and getAppWithCors below.
   * @returns {void}
   */
  ...
  /**
   * Helper: like getApp, but also mounts the REAL `cors` middleware (not a
   * mock) ahead of the catch-all, mirroring production ordering — in
   * lib/services/express.js `init()`, initMiddleware (which mounts cors) runs
   * before initModulesServerRoutes. Used to prove, by execution rather than
   * assumption, that a preflight OPTIONS request is answered by cors and
   * never reaches the app.all catch-all.
   * @returns {Promise<import('express').Express>} A ready-to-request Express app
   */
  const getAppWithCors = async () => {
    mockCommonDeps();
  ...
  // POST/PUT/PATCH/DELETE fell through to Express's default HTML
  // finalhandler instead of getting the same content-negotiated 404. It is
  // now registered with app.all — verify every non-GET verb on both an API
  // path and a non-API path.
  describe.each(['post', 'put', 'patch', 'delete'])('%s (unmatched, all verbs must 404 like GET — #3978)', (method) => {
  ...
  // #3978: app.all also matches OPTIONS, which is the one real risk the issue
  // calls out — a preflight request must still get cors' own answer, not our
  // 404. Two tests prove this by execution, not assumption: first that the
  // catch-all really does match OPTIONS when nothing else intercepts it
  // (i.e. the risk is real), then that production's actual middleware order
  // (cors mounted before routes) prevents it.
  test('OPTIONS /api/nope with no cors mounted → falls through to the catch-all (proves app.all matches OPTIONS)', async () => {
    const app = await getApp();

    const res = await request(app).options('/api/nope').expect(404);
  ...
  test('OPTIONS preflight is answered by the real cors middleware before the catch-all (production ordering, #3978)', async () => {
    const app = await getAppWithCors();
