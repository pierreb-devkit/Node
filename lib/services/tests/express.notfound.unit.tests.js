...
describe('express initModulesServerRoutes — content-negotiated 404 (#3975, #3978):', () => {
  ...
  /**
   * Mocks every heavy dependency initModulesServerRoutes needs. Used by getApp below.
   * @returns {void}
   */
  // Mocks transitively cover express.js's whole module scope (config, logger,
  // guides, requestId, posthog context, error tracker, analytics, policy) so
  // the real express.js module can be imported in isolation.
  ...
  /**
   * Extracts initModulesServerRoutes from express.js and mounts it on a fresh Express app.
   ...
   */
  // `withCors: true` mounts the REAL `cors` middleware (not a mock) ahead of
  // the catch-all, with the same order and config express.js's own `init()`
  // uses (there, initMiddleware — which mounts cors — runs before
