 ...
 * config.openapi.enable is still truthy. This closes the env-gate defect where
...
describe('express initApiSpec — env gate (spec off in non-dev envs):', () => {
  ...
  /**
   * Run initApiSpec under a given NODE_ENV with config.openapi.enable left truthy.
  ...
  test('DOES mount the spec under development (config.openapi.enable === true)', async () => {
  ...
  test('does NOT mount the spec in dev when config.openapi.enable is false', async () => {
    process.env.NODE_ENV = 'development';
    jest.unstable_mockModule('../../../config/index.js', () => ({
      default: { ...baseConfig, openapi: { enable: false } },
    }));
    const { default: expressService } = await import('../express.js');
    const app = buildMockApp();
    expressService.initApiSpec(app);
  ...
  test('DOES mount the spec in a production-grade env when config.openapi.public === true', async () => {
    // Opt-in: a consumer with intentionally-public API docs sets public true,
    ...
    jest.unstable_mockModule('../../../config/index.js', () => ({
      default: { ...baseConfig, openapi: { enable: true, public: true } },
    }));
    const { default: expressService } = await import('../express.js');
    const app = buildMockApp();
    expressService.initApiSpec(app);
