 ...
 */
const claimMigration = async (name, context) => {
  try {
    await migrationRepository.claim(name, context);
    return true;
  } catch (err) {
 ...
 * boot) is unchanged — this only runs on the success branch. Scoped to
 * `status:'running'` at the repository layer so a late success from a
 * runner whose claim was already cleared as stale can never overwrite a
 * newer runner's `'done'` record (#3992 follow-up).
...
/**
 * Remove a migration's claim record so it can be retried.
 * - With `context` ({pid, host}): ownership-scoped, via
 *   {@link migrationRepository.deleteClaim} — only removes the record if it
 *   is still the exact claim `context` identifies. Used by
 *   {@link runMigration}'s own failure paths so a late failure can never
 *   delete a DIFFERENT (newer) runner's record after this claim was already
 *   cleared as stale by another boot (#3992 follow-up).
 * - Without `context`: unscoped, via {@link migrationRepository.deleteByName}
 *   — used ONLY by {@link resolveRunningClaim} to clear crash residue left
 *   by a different, presumed-dead process; the caller does not own that claim.
 * @param {string} name - the migration filename
 * @param {{pid?: number, host?: string}} [context] - claim ownership context
 * @returns {Promise<object>} the deletion result
 */
const unclaimMigration = (name, context) =>
  context ? migrationRepository.deleteClaim(name, context) : migrationRepository.deleteByName(name);
...
const resolveRunningClaim = async (record, { graceMs, pollIntervalMs = DEFAULT_STALE_CLAIM_POLL_INTERVAL_MS }) => {
  ...
  while (current && current.status === 'running') {
    // startedAt is always set alongside status:'running' by repository.claim()
    // — but a manually-tampered or otherwise malformed record could still
    // carry a missing/unparsable startedAt. A non-finite age can't be
    // compared against graceMs, so treat it as immediately stale rather than
    // let `NaN >= graceMs` (always false) spin this loop forever on a ~1ms
    // setTimeout (setTimeout coerces a NaN delay to ~1ms).
...
const runMigration = async (filePath, executed) => {
  ...
  // Atomically claim the migration to prevent concurrent execution. This
  // process's own pid/host identify exactly the claim record it is about to
  // insert — captured once so every failure-path unclaim below can be scoped
  // to it (#3992 follow-up: a late failure must never delete a claim this
  // process doesn't own, e.g. one a newer runner already re-claimed/completed
  // after this claim was cleared as stale by another boot).
  const claimContext = { pid: process.pid, host: os.hostname() };
  const claimed = await claimMigration(name, claimContext);
  if (!claimed) {
  ...
  try {
    ...
    await unclaimMigration(name, claimContext);
    throw err;
  }
