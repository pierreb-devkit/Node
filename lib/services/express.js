...
const initApiSpec = (app) => {
  ...
  // config.openapi.enable is still truthy — opt-OUT by default in non-dev,
  // UNLESS the consumer explicitly opts into serving it publicly via
  // config.openapi.public === true (e.g. a project whose API docs are
  // intentionally public). The opt-in narrows the gate, never widens it: enable
  // must still be truthy.
  if (config.openapi.enable && (!configHelper.isProd() || config.openapi.public === true)) {
    if (!config.files.openapi || config.files.openapi.length === 0) {
      logger.warn('[openapi] no OpenAPI spec files configured — skipping API docs');
      return;
    }
    ...
    const contents = config.files.openapi
      .map((filePath) => {
        try {
          ...
          throw new Error(`[openapi] failed to load ${filePath}: ${err.message}`, { cause: err });
        }
      })
      .filter(Boolean);
    ...
    if (Object.keys(spec).length === 0) {
      logger.warn('[openapi] all YAML files were skipped — no valid spec to serve');
      return;
    }
    ...
    if (guides.length > 0) {
      logger.info(`[openapi] merged ${guides.length} markdown guide(s) into API reference`);
    }
...
const initMiddleware = (app) => {
  ...
  // Honor the configured value (a hop count like 1, a subnet, or a boolean).
  // Forwarding the raw config avoids collapsing a bounded hop count to the
  // fully-permissive `true`, which would let a client spoof `req.ip` via a
  // rotating leftmost X-Forwarded-For and bypass the IP-keyed rate limiter.
  if (config.trust && config.trust.proxy) app.set('trust proxy', config.trust.proxy);
...
const initModulesServerRoutes = async (app) => {
  ...
  // Registered with app.all (not app.get, #3978) so every unmatched verb
  // (POST/PUT/PATCH/DELETE/...) gets the same JSON/HTML negotiation instead
  // of falling through to Express's default HTML finalhandler. This also
  // matches OPTIONS, but the cors middleware (initMiddleware) is mounted
  // before routes and always answers preflight requests itself without
  // calling next(), so a real preflight never reaches this handler.
  /**
   * Content-negotiated 404 for any unmatched path/verb.
   * @param {import('express').Request} req - Incoming request
   * @param {import('express').Response} res - Outgoing response
   * @returns {import('express').Response} The 404 response (JSON for API paths/JSON-accepting clients, HTML otherwise)
   */
  app.all('/{*path}', (req, res) => {
    ...
    return res.status(404).send('<center><br /><h1>404</h1><h3>Not Found</h3></center>');
  });
};
