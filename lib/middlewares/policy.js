...
const resolveSubject = (req) => {
  ...
  if (req.apiKey) return { subjectType: 'DeveloperKey', document: normalizeForCasl(req.apiKey) };
  if (req.webhook) return { subjectType: 'DeveloperWebhook', document: normalizeForCasl(req.webhook) };
  // Billing and developers routes use req.organization for context but authorize via route-derived subjects
  if (req.organization && !req.route?.path?.startsWith('/api/billing') && !req.route?.path?.startsWith('/api/developers')) {
...
const isAllowed = async (req, res, next) => {
  ...
  // API-key authenticated requests bypass CASL — the key already scopes to an organization
  if (req.apiKeyAuth && req.organization) return next();
...
const deriveSubjectType = (routePath) => {
  ...
  if (routePath.startsWith('/api/developers/keys')) return 'DeveloperKey';
  if (routePath.startsWith('/api/developers/webhooks')) return 'DeveloperWebhook';
