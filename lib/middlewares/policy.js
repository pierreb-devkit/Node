...
/**
 * Mapping of exact route paths to CASL subject types for user-related routes.
 * Self-service routes use 'UserAccount', admin routes use 'UserAdmin',
 * and the base '/api/users' uses 'UserSelf' to split read (admin) from update/delete (user).
...
const USER_ROUTE_SUBJECTS = {
  ...
  '/api/users/accounts': 'UserAccount',
...
const deriveSubjectType = (routePath) => {
  ...
  // Check exact user route matches first
  if (USER_ROUTE_SUBJECTS[routePath]) return USER_ROUTE_SUBJECTS[routePath];
  // Remaining user routes (/:userId, /page/:userPage) are admin
  if (routePath.startsWith('/api/users')) return 'UserAdmin';
  return null;
};
 ...
 * Each policy file is expected to export named functions (e.g. taskAbilities, taskGuestAbilities).
...
const discoverPolicies = async (policyPaths) => {
  for (const policyPath of policyPaths) {
    ...
    for (const [key, value] of Object.entries(mod)) {
      if (typeof value === 'function' && key.endsWith('Abilities') && !key.endsWith('GuestAbilities')) {
        entry.abilities = value;
      }
      if (typeof value === 'function' && key.endsWith('GuestAbilities')) {
