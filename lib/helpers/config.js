 ...
 * non-dev environments. Do not add project-specific placeholders here: keep
 * any project placeholder under 32 characters, so the length check in
 * isJwtSecretWeak below rejects it outside dev regardless of this list.
 * @readonly
 */
 ...
 *
 * Globs each category of files (OpenAPI YAML, markdown guides, mongoose
...
const initGlobalConfigFiles = async (assets) => {
  ...
  files.openapi = await getGlobbedPaths(assets.allYaml); // Setting Globbed module OpenAPI yaml files
