...
const logConfiguration = async () => {
  ...
  logger.info(chalk.green(`Environment:     ${process.env.NODE_ENV ? process.env.NODE_ENV : 'develoment'}`));
  ...
  if (process.env.NODE_ENV !== 'test') {
    try {
      ...
      logger.warn(chalk.yellow(`  SaaS readiness check failed: ${err.message}`));
    }
  }
};
