...
const logConfiguration = async () => {
  ...
  if (process.env.NODE_ENV !== 'test') {
    try {
      ...
      // The billing row only asserts the key is PRESENT. When it is, spend one request proving it
      // actually works, so a revoked or expired key is visible here instead of on the first charge.
      // Gating on the row already being `ok` is also what keeps billing removable: without it the
      // row is `warning` and no billing code is ever imported.
      const billingRow = checks.find((c) => c.category === 'billing');
      if (billingRow?.status === 'ok') {
        try {
          const { default: getStripe, validateStripeKey } = await import('../modules/billing/lib/stripe.js');
          Object.assign(billingRow, await validateStripeKey(getStripe()));
        } catch (err) {
          // Own try/catch: a missing module or a network failure must not take the whole summary down.
          // validateStripeKey() never throws, so only an import or getStripe() failure reaches here.
          Object.assign(billingRow, {
            status: 'warning',
            message: `Stripe key could not be validated (${err?.message ?? 'unknown error'})`,
          });
        }
      }
      ...
      checks.forEach((c) => {
        let icon;
        if (c.status === 'error') icon = chalk.red('ERR');
        else if (c.status === 'ok') icon = chalk.green('OK');
        else icon = chalk.yellow('WARN');
        const line = `  ${icon}  ${c.category.padEnd(12)} ${c.message}`;
        // A failure logged at info level is the silence this check exists to break.
        if (c.status === 'error') logger.error(line);
