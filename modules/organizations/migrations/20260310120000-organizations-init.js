  ...
  if (Task) {
    ...
    for (const task of tasksWithoutOrganization) {
      ...
      const orgId = taskUser?.currentOrganization
        || (await Membership.findOne({ userId: task.user, $or: [{ status: 'active' }, { status: { $exists: false } }] }).lean())?.organizationId;
      if (orgId) {
  ...
  if (Upload) {
    ...
    for (const upload of uploadsWithoutOrganization) {
      ...
      const orgId = uploadUser?.currentOrganization
        || (await Membership.findOne({ userId: upload.metadata.user, $or: [{ status: 'active' }, { status: { $exists: false } }] }).lean())?.organizationId;
      if (orgId) {
