...
- [2026-05-05] repository: Repository.update(doc) doing `new Model(doc).save()` rewrites the full document from in-memory state, silently clobbering any concurrent patchOne($set) that landed after the read -> convert all callers that share a document with concurrent patchers to patchOne($set) instead; see trawl_node #1115 #1116 #1118 + Devkit-Node#3605
